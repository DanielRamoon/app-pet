...
import { View, Text, Image, Dimensions, Alert, RefreshControl, FlatList } from "react-native";
...
export default function MyAllPosts() {
	...
	const getNextMyWalks = useCallback(
		// async (page?: number) => {
		...
		// 	}
		// 	setGettingMore(true);
		// 	const _walks = await walkService.getMyWalks(page || 0);
		// 	setGettingMore(false);

		// 	if (_walks.messages || !_walks.data) {
		...
		// 	// Usa um Map para garantir a unicidade dos objetos pelo ID
		// 	const walkMap = new Map();
		// 	for (const walk of newArray) {
		// 		walkMap.set(walk.id, walk);
		// 	}
		// 	const filtered = Array.from(walkMap.values());
	...
	const renderItem = ({ item, index }: { item: IPetWalk; index: number }) => (
		<Post petWalk={item} isLast={index === walks.length - 1} isMy={true} />
	);
